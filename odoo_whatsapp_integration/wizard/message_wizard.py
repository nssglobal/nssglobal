...
class SendMessage(models.TransientModel):
    ...
    resource_id  = fields.Many2one('resource.resource', string="Recipient Name Resource", default=lambda self: self.env[self._context.get('active_model')].browse(self.env.context.get('active_ids')).resource_id)
    mobile_no = fields.Char(related='resource_id.mobile_no', required=True)
